# 3.DEG prediction and attributing transcriptional changes

**Tichr**  `studyDEG` module can be used to identify related genes and functional elements in a comparison. 

In detail, the purpose of this analysis if for identifying the involved genes and elements that are related to specific treatment. Therefor, two sets of multi-omics data is required - one set for control group, and another set for treated group. And **DEGs** will be found between these two sets.
...
``` python
diffobj = studyDEG.DiffEvent(RgDF_Ctrl_file,RgxDF_Ctrl_file,RgDF_Treat_file,RgxDF_Treat_file, outdir="")
...
PRC of predict DEGs by `Rg` value, under different threshold of top quantile genes by the `selectGeneType` parameter. **This figure illustrates the performance of DEG prediction under different thresholds.** This holds true for the following other two figures as well.
...
### Select the related functional sites by multiple criteria

The `diffEP`function can be used to select related functional sites. The main purpose is to output the related sites if they are related to the DEGs in the comparisons
...
If you have data on multiple epigenetic modifications, Tichr allows you to combine them to predict Differentially Expressed Genes (DEGs ).

We have prepared test data to demonstrate this functionality. Here is an example:

### Prepare data

``` python
from tichr import multiEpiPredict
