# 4.Regulation-transcription concordance analysis

The **Tichr**  `treatment` module allows you to perform correlation analysis between **Gene Rg** and **Gene Expression**. This analysis consists of two parts:

1. The correlation between **pre-treatment Rg**and **pre-treatment gene expression**.
2. The correlation between the **change in Rg (post-treatment vs. pre-treatment)**and the **change in gene expression (post-treatment vs. pre-treatment)**.

Here is an example to show how to use  **Tichr**  `treatment` modulel for analysis. 
...
```python
corrrg, corrdelta = treatment.corrfunc(datadir,pair_list, corrtype="pearson",suffix="_RgDf.tsv")
```
...
```python
treatment.plot_correlation_scatter(corrrg, corrdelta, factorlist, title="RPE_siNIPBL")
```
...
``` python
rg_merged,rgx_merged = context.mergeDF(rgCtrl_path,rgTreat_path,rgxCtrl_path,rgxTreat_path,minRgx=0.1,minRgxRatio=0.01)
```
...
``` python
context.prepare_select_by_rank(rg_merged, basedon="rg",filetype="pandas",                        
