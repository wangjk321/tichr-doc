# 2. Characterizing enhancer-promoter interactions

The **Tichr** `siteToGene`module can be used to predict Enhancer-Promoter(E-P) Pairs based on `RgX` results. To predict E-P interactions, you must have a gold standard for benchmarking the `RgX` results. You can use the promoter-capture Hi-C, or other related methods to get the correct data.
...
``` python
showROC(matched[truecol-1],matched.iloc[:,-1],"match1",outname="match1_roc.pdf")
```
...
``` python
matched2 = matchgold(f"{datadir}/K562_RgxDF_type2.tsv",golddf,outname,goldcol,
          percent=False,withhead=withhead,returnDF=True)

matched3 = matchgold(f"{datadir}/K562_RgxDF_type3.tsv",golddf,outname,goldcol,
          percent=False,withhead=withhead,returnDF=True)
```
...
diffrentThresh(rgxfile,rgfile,golddf,goldcol,outdir,tpmdir,
