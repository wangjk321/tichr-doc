...
- `quantile_method = None`  
  ...
  - `'rankpercent'` — use the *quantile* column in the reference  
  - `'rank'` — use the *rank* column in the reference
...
- **`hicDataType`**: Workflow depends on the Hi-C input format (dense / raw dense / raw sparse).  
  **In most cases, `'rawhic_sparse'` is recommended. Provide a `.hic` file in Step 1.**
  ...
  - **`rawhic_dense`**: 
    - Suitable for “non-standard” `.hic` files (e.g., generated by JuicerTools > 2.0). 
...
- **`contactNorm`**: Additional user-defined normalization strategy for Hi-C–informed weighting:  
...
***Processing genes: 100%|█| 2412/2412 [00:12<00:00, 196.6
***Finished
```
...
tichrobj.computeAllGene(weightType,
                fixedFunctionType='exponential',halfDistance=10000,
                 ...
                 userWeight=None， noise_ratio=0,noise_quantile=0):
```
...
## Step5.5 Manipulate Tichr object
...
## Step6. Adjust S2G regulation

TICHR can optionally perform adjustments according to the following rules. For more decription, see our original paper:

1. `adjRaw`: **Raw regulatory values instead of ratios**. The raw RgX value is used instead of the within-gene ratio, which is the default behavior in Tichr. 
2. `adjrank`: **incorporating gene expression concordance**. For each gene–site pair 𝑖, the RgX value is scaled by a penalty term that reflects the absolute rank difference between RgX and gene expression (TPM) across all pairs. 
