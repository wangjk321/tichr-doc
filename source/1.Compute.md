...
genebedfile=f"{datadir}/all_coding_gene.tsv"
...
***Temporary directory created at: tichr_tmp_hEAgHlKBbZ
...
***Processing genes: 100%|█| 12298/12298 [00:24<00:00, 501
...
#reference
gtfile=f"{datadir}/hg38_gt.tsv"
refGeneFile=f"{datadir}/all_coding_gene.tsv"
#sites and genes
candidatesite=f"{datadir}/candidatePeak.bed"
...
- `ifTSSrange`: Defines the promoter region as transcription start site (TSS) ± this range. 
...
- `S2Gmax`: Maximum distance (in base pairs) allowed between a peak and a gene for linking. 
  ...
  - 5 Mb can be used to capture ultra–long-range interactions.
...
- `fixPeakWidth`: Applicable only for given `user-defined-bed3` candidate sites.
  - If set to True, each peak’s width is fixed to 500 bp by centering and extending ±250 bp.
  - default: True

- `only_promoter_area`: Only be used when `candidatesite` is set to be “onlypromoter”. 
  - Default: 100
...
tichrobj.makeSiteBdg(coverageMethod="coverageBed",spmr = True,multiBAMmerge='mean',file_type="bigWig")
...
tichrobj.proceessHiC(50000,'rawhic_sparse',"VC_SQRT",
                   threads=12,contactNorm='no_further')
```
...
- `contactNorm`: Could be `default`, `abc`, `oe`, `0to1`,`total`. 
